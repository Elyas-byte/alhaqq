...
const Player = () => {
  ...
  }, [user, track.id, db]); // Include db in the dependency array
      ...
      if (likedPlaylistDoc.exists()) {
        ...
        if (isLiked) {
          ...
          const updatedSongs = likedSongs.filter(
            (songId) => songId !== track.id
          );
          await updateDoc(likedPlaylistRef, { songs: updatedSongs });
        } else {
          // Add song to liked songs
          await updateDoc(likedPlaylistRef, {
            songs: [...likedSongs, track.id],
          });
        }
  ...
  return (
    <div className="h-[10%] bg-[#0D0D0D] flex justify-between items-center text-white px-4">
      <div className="hidden lg:flex items-center gap-4">
        <TrackImage className="w-12 h-12" src={track.image} alt={track.name} />
        <div>
          <p>{track.name}</p>
          <p>{track.desc}</p>
        </div>
      ...
      <div className="hidden lg:flex items-center gap-2 opacity-75">
        ...
        <img
          onClick={mute}
          className="w-5 cursor-pointer"
          src={isMute() ? assets.volume_icon : assets.mute}
          alt=""
        />
        <div
          ref={volumeBg}
        ...
        >
          <hr
            ref={volumeBar}
            className="h-1 border-none w-[100%] bg-red-800 rounded-full transition-all duration-300"
          />
        </div>
