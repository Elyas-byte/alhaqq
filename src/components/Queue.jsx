...
const QueueItem = ({ song, index, moveSong, onDelete }) => {
    ...
    return (
        <div
            ref={dragDropRef}
            className={`grid grid-cols-4 items-center rounded p-2 mb-2 cursor-move mb-1 transition-all duration-200 ${isDragging ? 'opacity-50' : 'opacity-100'
        ...
        >
            ...
            <span
                ...
                onClick={() => {
                    const artist = artistsData.find(artist => artist.name === song.desc);
                    if (artist) {
            ...
            <span className="text-sm font-medium hover:underline" onClick={() => navigate(`/album/${song.album}`)}>{song.albumN}</span>
            <button
                onClick={() => {
                    if (track.id !== song.id) {
                        onDelete(song.id);
...
const Queue = () => {
    ...
    const { queue, setQueue, removeFromQueue } = useContext(PlayerContext);
