  ...
  return (
    <>
          ...
          <div className="col-span-2 flex items-center space-x-4">
            ...
            <div className="flex flex-col text-[#EAEAEA] mt-[25%]">
              ...
              <p className="mt-2 text-sm md:text-base">
                {artistData.followers} {t("followers")} - {artistData.albums}{" "}
                {t("albums")}
              </p>
            </div>
          </div>
      ...
      <div className="max-w-6xl mx-auto p-6 md:p-10">
        <h3 className="text-2xl md:text-3xl font-bold text-[#EAEAEA] mb-6">
          {t("t_albums")}
        </h3>
        <div className="flex overflow-auto">
          {albumsData
            .filter(
              (album) =>
                album.artist === artistData.id ||
                album.artist2 === artistData.id ||
                album.artist3 === artistData.id
            )
            .map((album, index) => (
              <AlbumItem
                key={index}
      ...
      <div className="max-w-6xl mx-auto p-6 md:p-10">
        ...
        <div className="flex overflow-auto">
          {songsData
            ...
            .map((item) => (
              <SongItem
                key={item.id} // Using stable item.id as key instead of index
                name={item.name}
                desc={item.desc}
                id={item.id}
                image={item.image}
