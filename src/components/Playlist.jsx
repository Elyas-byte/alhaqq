...
const Playlist = () => {
  ...
  const { playWithId, addToQueue } = useContext(PlayerActionsContext); // Add addToQueue to context
  ...
  const fetchPlaylistData = async () => {
    try {
      ...
      if (playlistSnapshot.exists()) {
        ...
        const matchedSongs = playlist.songs.map(
          (songId) =>
            songsData.find((song) => song.id === songId) || {
              ...
              desc: "Unknown",
              duration: "Unknown",
              image: assets.default_song_image,
            } // Add image fallback
        );
